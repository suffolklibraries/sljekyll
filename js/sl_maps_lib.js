(function (window, undefined) {
    var MapsLib = function (options) {
        ...
        this.myOptions = {
            ...
            scrollwheel: false, // LP 20160717 disable scroll zoom
        ...
        // maintains map centerpoint for responsive design LP 20170721 remove
    ...
    MapsLib.prototype.submitSearch = function (whereClause, map) {
        var self = this;
        self.map.data.loadGeoJson('libraries.json', null, function() {
            self.filterMarkers(self.map);
        });
    };

    MapsLib.prototype.filterMarkers = function(map) {
        var self = this;        
        if (self.currentPinpoint) {
            map.data.forEach(function(marker) {
                const lat = marker.getGeometry().get().lat();
                const lng = marker.getGeometry().get().lng();
    
                const searchArea = new google.maps.Circle({
                    center: self.currentPinpoint,
                    radius: parseInt(self.searchRadius)
                });
    
                const distance = google.maps.geometry.spherical.computeDistanceBetween(new google.maps.LatLng(lat, lng), searchArea.getCenter());
